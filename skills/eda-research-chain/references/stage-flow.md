...
Use this file after `SKILL.md` has selected `eda-research-chain`.

`eda-research-chain` remains the workflow owner across all stages below.
Use `eda-loop` only for the specific execution/validation stages that need governed execution wrapping.
